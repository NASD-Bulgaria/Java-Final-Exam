		...
		if (cookies != null) {
			for (Cookie cookie : cookies) {
				if (cookie.getName().equals("user")) {
					...
					JSONObject jo = (JSONObject) new JSONObject().append("hash", hc.toString()).append("status", "ok");
					
					PrintWriter out = response.getWriter();
					out.println("<!DOCTYPE HTML ><HTML><HEAD><TITLE>Login Post</TITLE></HEAD><BODY>"
							+ "<form method = \"Post\" action=\"http://localhost:8080/PlayerProject/LogoutServlet\""
							+ "<H1>Deposit</H1><p>" 
							+ "<p>" + jo + "<p>" 
							+ request.getParameter("dep") 
							+ "<p>"
