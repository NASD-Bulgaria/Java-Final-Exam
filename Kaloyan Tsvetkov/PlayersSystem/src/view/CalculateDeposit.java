		...
		if (tokenRequest.equals(tokenDB) && (request.getSession().getAttribute("tokenSesion"))!=null) {
			if (!sumText.isEmpty()) {
				...
				if (sum > 0) {
					...
					boolean result;
					synchronized (this) {
						result = Queries.depositSum(id, sum);
					}
					
					if (result) {
						JSONObject jsonOb = new JSONObject().put("token", "hash").put("ammount", "100");
						out.println(output
								+ "<html><head><title>Deposit Money</title></head>"
								+ "<body bgcolor=\"#f0f0f0\"><BR><BR>"
								+ "<form action=\"/PlayersSystem/PlayerServlet\" method=\"POST\">"
								+ "<CENTER>"
								+ "<p>" + jsonOb.toString() + "</p><BR><BR>"
								+ "This sum " + sum + "<BR>was deposit to your balance.<BR>"
					...
					out.println(output
							...
							+ "<form action=\"/PlayersSystem/PlayerServlet\" method=\"POST\">"
							+ "<CENTER>"
							+ "To you can't deposit " + sum
							+ "<BR>the sum must be > 0.00." + "<BR>"
							+ "<input type=\"hidden\" name=\"compID\" value=\"" + id + "\" />"
							+ "<input type=\"hidden\" name=\"token\" value=\"" + tokenRequest + "\" />"
							+ "<input type=\"submit\" value=\"Back\" /><BR><BR>"
							+ "</CENTER>" + "</form></body></html>");
				...
				out.println(output
					...
					+ "<body bgcolor=\"#F8ECEE\">\n"
					+ "<form action=\"/PlayersSystem/PlayerServlet\" method=\"POST\">"
					+ "<CENTER>" + "Must enter sum to deposit" + "<BR><BR>"
					+ "<input type=\"hidden\" name=\"compID\" value=\"" + id + "\" />"
					+ "<input type=\"hidden\" name=\"token\" value=\"" + tokenRequest + "\" />"
